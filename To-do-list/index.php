...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['first_name'], $_POST['last_name'], $_POST['email'], $_POST['password'], $_POST['confirm_password'])) {
    ...
    $verification_token = bin2hex(random_bytes(16)); // Generates a secure random token
    $insert_stmt = $pdo->prepare("
        INSERT INTO users (first_name, last_name, email, password, created_at, updated_at, verification_token, is_verified) 
        VALUES (:first_name, :last_name, :email, :password, :created_at, :updated_at, :verification_token, 0)");
    ...
    if ($insert_stmt->execute()) {
        ...
        try {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = 'todoappmema@gmail.com'; // your Gmail
            $mail->Password = 'nejt eywx cnzk rzrq'; // your Gmail App Password
            $mail->SMTPSecure = 'tls';
            $mail->Port = 587;

            $mail->setFrom('todoappmema@gmail.com', 'To-Do Plan');
            $mail->addAddress($email, $first_name . ' ' . $last_name);
            $mail->Subject = 'Verify Your Email';
            $mail->Body = "Click this link to verify your email:\n\n" .
                          "http://localhost/Projects/Portfolio/To-do-list/database/verify.php?token=$verification_token";

            $mail->send();

            $_SESSION['response'] = ['success' => true, 'message' => 'User registered successfully. Please check your email to verify your account.'];
        } catch (Exception $e) {
            $_SESSION['response'] = ['success' => false, 'message' => 'Failed to send verification email.'];
        }
    } else {
                ...
                <div class="col-md-10">
                    <div class="row border rounded p-4 shadow-lg bg-light">
                        ...
                        <div class="col-md-6">
                            ...
                            <form action="partials/send-message.php" method="POST">
                                <div class="mb-3">
